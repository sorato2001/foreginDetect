...
from .ftp_image_detector import get_vision_analyzer
...
class EventFinalizer:
    ...
    def _finalize_video_sync(
        ...
        try:
            ...
            active_event.analysis_result = analyzer.analyze_video(video_path)
            final_status = self._resolve_event_status(active_event.analysis_result)
            logger.info(
                "Video analysis finished for %s with status=%s",
                active_event.camera_id,
                final_status,
            )
    ...
    def _finalize_image_sync(
        ...
        try:
            ...
            analysis_result = analyzer.analyze_image(
                ...
                save_annotated_images=self.save_annotated_images,
            )
            active_event.analysis_result = analysis_result
            active_event.annotated_image_path = analysis_result.get("annotated_image_path")
            final_status = self._resolve_event_status(analysis_result)
            logger.info(
